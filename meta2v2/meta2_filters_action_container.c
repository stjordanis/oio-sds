		...
		struct list_params_s *lp, GSList *headers)
{
	...
	void s3_list_end_cb(struct sqlx_sqlite3_s *sq3) {
		...
		if (truncated) // Already truncated
			goto end;
		if (!*shard_info->upper) // Last shard
			goto end;
		if (lp->prefix
				// The prefix is before the last object of this shard
				&& g_strcmp0(lp->prefix, shard_info->upper) <= 0
				// The last object of this shard desn't start with the prefix
				&& !g_str_has_prefix(shard_info->upper, lp->prefix))
			goto end;
		if (lp->marker_end
		...
		struct gridd_reply_ctx_s *reply)
{
	...
	struct oio_url_s *url = meta2_filter_ctx_get_url(ctx);
	struct meta2_backend_s *m2b = meta2_filter_ctx_get_backend(ctx);
	gchar *queries = meta2_filter_ctx_get_input_udata(ctx);

	GError *err = meta2_backend_update_shard(m2b, url, queries);
	if (!err) {
		return FILTER_OK;
	}
	meta2_filter_ctx_set_error(ctx, err);
