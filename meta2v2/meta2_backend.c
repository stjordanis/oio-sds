...
m2b_close(struct sqlx_sqlite3_s *sq3, struct oio_url_s *url)
{
	...
	if (err) {
		...
		GRID_ERROR("Abort sharding: (%d) %s", err->code, err->message);
		g_clear_error(&err);
		goto exit;
	}
		...
		enum m2v2_open_type_e how, struct sqlx_sqlite3_s **result)
{
	...
	if (!sq3->sharding_queue) {
		gint64 sharding_state_timestamp = sqlx_admin_get_i64(sq3,
			M2V2_ADMIN_SHARDING_STATE_TIMESTAMP, 0);
		if (!sharding_state_timestamp) {
			// Should never happen
			// TODO(adu): Abort sharding
			GRID_ERROR("Abort sharding: (%d) %s", err->code, err->message);
			g_clear_error(&err);
		...
		err = _connect_to_sharding_queue(url,
				sharding_queue, sharding_state_timestamp, &beanstalkd);
		if (err) {
			// TODO(adu): Abort sharding
			GRID_ERROR("Abort sharding: (%d) %s", err->code, err->message);
			g_clear_error(&err);
		...
		const gchar *path, gboolean read_only)
{
	...
	shard_info_str = sqlx_admin_get_str(sq3, M2V2_ADMIN_SHARDING_SHARD_INFO);
	if (!shard_info_str) {
		err = SYSERR("Not a shard");
		goto end;
	}

	// It's a shard, check range of shard
	if (!path) {
	...
	if (err) {
		g_prefix_error(&err, "Failed to check range of shard: ");
	}
...
_redirect_to_shard(struct sqlx_sqlite3_s *sq3, const gchar *path)
{
	...
	shard_ranges_str = sqlx_admin_get_str(sq3,
			M2V2_ADMIN_SHARDING_SHARD_RANGES);
	if (!shard_ranges_str) {
		// No sharding
		goto end;
	}
	...
	struct shard_range_s *shard_range = shard_ranges_get_shard_range(
			shard_ranges, path);
	if (!shard_range) {
		err = SYSERR("No shard found");
		goto fail;
	}
		...
		struct oio_url_s *url, m2_onbean_cb cb, gpointer u0)
{
	...
	struct sqlx_sqlite3_s *sq3 = NULL;
	struct sqlx_repctx_s *repctx = NULL;
	err = m2b_open_for_object(m2, url, M2V2_OPEN_MASTERONLY | M2V2_OPEN_ENABLED, &sq3);
	if (!err) {
		EXTRA_ASSERT(sq3 != NULL);
		if (!(err = sqlx_transaction_begin(sq3, &repctx))) {
	...
	if (!err) {
		...
		if (!(err = _transaction_begin(sq3, url, &repctx))) {
			sqlx_admin_set_i64(sq3, M2V2_ADMIN_SHARDING_STATE,
					CONTAINER_TO_SHARD_STATE_SAVING_WRITES);
			sqlx_admin_set_i64(sq3,
					M2V2_ADMIN_SHARDING_STATE_TIMESTAMP, timestamp);
			sqlx_admin_set_str(sq3,
					M2V2_ADMIN_SHARDING_QUEUE, queue_url);
			m2db_increment_version(sq3);
			err = sqlx_transaction_end(repctx, err);
			if (!err)
				m2b_add_modified_container(m2b, sq3);
		...
		struct shard_info_s **pshard_info)
{
	...
	if (!shard_info_str) {
		...
		// It's a shard
		err = shard_info_decode(shard_info_str, pshard_info);
	}
