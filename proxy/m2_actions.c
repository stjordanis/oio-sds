		...
		gchar **dest_properties)
{
	...
	if (strcmp(src_cid, dest_cid) == 0) {
		err = BADREQ("The destination must either have a different name or "
...
_body_extract(gpointer ctx, guint status UNUSED, MESSAGE reply)
{
	...
	gchar *out = NULL;
	GError *err = metautils_message_extract_body_string(reply, &out);
	if (err) {
		GRID_DEBUG("Callback error: (%d) %s", err->code, err->message);
		g_error_free(err);
		return FALSE;
		...
		struct json_object *j UNUSED)
{
	...
	PACKER_VOID(_pack) {
		return m2v2_remote_pack_REPLACE_SHARDING(args->url, args->rq->body,
				DL());
	};
	err = _resolve_meta2(args, _prefer_master(), _pack, NULL, NULL);
		...
		struct json_object *j UNUSED)
{
	...
	PACKER_VOID(_pack) {
		return m2v2_remote_pack_SHOW_SHARDING(args->url, DL());
	};
